...
int main(int argc, char** argv){
	...
	// Create a 'MultiViewShapeAdjustmentProblem' instance to hold the BA problem
	MultiViewShapeAdjustmentProblem myProblem;
	...
	X_bar_initial = new double[numViews*3*numPts];
	for(int i = 0; i < numViews*3*numObs; ++i){
	...
	// Get the top 42 eigenvectors of the wireframe
	...
	double *rotAngleAxis = new double[3*numViews];
	for(int v = 0; v < numViews; ++v){
		ceres::RotationMatrixToAngleAxis(rot + v*9, rotAngleAxis + v*3);
	}
	...
	for(int v = 0; v < numViews; ++v){
		...
		for(int i = 0; i < numObs; ++i){
			...
			// std::cout << "curEigVec: ";
			for(int j = 0; j < 42; ++j){
				curEigVec[3*j+0] = V[3*numObs*42*v + 3*numObs*j + 3*i + 0];
				curEigVec[3*j+1] = V[3*numObs*42*v + 3*numObs*j + 3*i + 1];
				curEigVec[3*j+2] = V[3*numObs*42*v + 3*numObs*j + 3*i + 2];
			...
			ceres::CostFunction *lambdaError = new ceres::AutoDiffCostFunction<LambdaReprojectionError, 2, 3, 42>(
					new LambdaReprojectionError(X_bar+3*numObs*v+3*i, observations+2*numObs*v+2*i, curEigVec, K, observationWeights[numObs*v+i], trans+3*v));




			// Add a residual block to the problem
			problem.AddResidualBlock(lambdaError, new ceres::HuberLoss(0.8), rotAngleAxis + 3*v, lambdas);
		...
		problem.SetParameterBlockConstant(rotAngleAxis + 3*v);
	}
	...
	for(int v = 0; v < numViews; ++v){
		for(int i = 0; i < numPts; ++i){
			double temp[3];
			temp[0] = X_bar_initial[3*numPts*v+3*i];
			temp[1] = X_bar_initial[3*numPts*v+3*i+1];
			temp[2] = X_bar_initial[3*numPts*v+3*i+2];

			for(int j = 0; j < 42; ++j){
				temp[0] += lambdas[j]*V[3*numObs*42*v + 3*numObs*j + 3*i + 0];
				temp[1] += lambdas[j]*V[3*numObs*42*v + 3*numObs*j + 3*i + 1];
				temp[2] += lambdas[j]*V[3*numObs*42*v + 3*numObs*j + 3*i + 2];
				// std::cout << V[3*numObs*j + 3*i + 0] << " " << V[3*numObs*j + 3*i + 1] << " " << \
				// 	V[3*numObs*j + 3*i + 2] << std::endl;
			}

			// double temp1[3];
			ceres::AngleAxisRotatePoint(rotAngleAxis + 3*v, temp, temp);
			// *temp = *temp1;
			temp[0] += trans[3*v + 0];
			temp[1] += trans[3*v + 1];
			temp[2] += trans[3*v + 2];
